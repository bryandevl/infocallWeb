...
class CreateValidataPeopleCompanyTable extends Migration {
	...
	public function up()
	{
		Schema::create('validata_people_company', function(Blueprint $table)
		{
			...
			$table->bigInteger('validata_people_id')->nullable()->index('IDX_VALIDATA_PEOPLE_COMPANY__VALIDATA_PEOPLE_ID');
			$table->bigInteger('validata_company_id')->nullable()->index('IDX_VALIDATA_PEOPLE_COMPANY__VALIDATA_COMPANY_ID');
			$table->string('position', 200)->nullable()->index('IDX_VALIDATA_PEOPLE_COMPANY__POSITION');
			$table->timestamps(10);
			$table->softDeletes();
	...
	{
		Schema::drop('validata_people_company');
	}
