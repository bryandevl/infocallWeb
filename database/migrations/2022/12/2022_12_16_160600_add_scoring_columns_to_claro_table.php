...
class AddScoringColumnsToClaroTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::table('claro', function ($table) {
            $table->string("origen_data", 100)
                ->index("IDX_CLARO__ORIGEN_DATA")
                ->after("numero")
                ->nullable();
            $table->date("fecha_data")
                ->index("IDX_CLARO__FECHA_DATA")
                ->after("origen_data")
                ->nullable();
            $table->string("plan", 200)
                ->index("IDX_CLARO__PLAN")
                ->after("fecha_data")
                ->nullable();
            $table->date("fecha_activacion")
                ->index("IDX_CLARO__FECHA_ACTIVACION")
                ->after("plan")
                ->nullable();
            $table->string("modelo", 200)
                ->index("IDX_CLARO__MODELO")
