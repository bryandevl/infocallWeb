<template>
    <form
        ...
        <div 
            ...
            v-show="form.succeeded"
            id="result"
        > {{
            trans('adminlte_lang_message.loggedin') }} <i class="fa fa-refresh fa-spin"></i> {{
            trans('adminlte_lang_message.entering') }}
        </div>
        <transition name="fade">
            <label class="help-block label-danger" v-if="form.withMessageError" v-text="form.messageError"></label>
        </transition>
        ...
        <!--<div class="form-group has-feedback" :class="{ 'has-error': form.errors.has('username') }" v-else>
            ...
            <transition name="fade">
                <span class="help-block" v-if="form.errors.has('username')" v-text="form.errors.get('username')"
                      id="validation_error_username"></span>
            </transition>
        </div>-->
            ...
            <div class="col-xs-6">
                <div class="checkbox icheck">
                    <label>
                        ...
                        trans('adminlte_lang_message.remember') }}
                    </label>
                </div>
            </div>
            <div class="col-xs-6">
                <button type="submit" class="btn btn-primary btn-block btn-flat"
                        v-text="trans('adminlte_lang_message.buttonsign')" :disabled="form.errors.any()"><i
                        v-if="form.submitting" class="fa fa-refresh fa-spin"></i></button>
            </div>
        </div>
    </form>
</template>
            ...
            submit() {
                this.form.post('/login')
                    .then(response => {
                        ...
                        if (parseInt(response.data.rst) == 2) {
                            this.form.withMessageError = true
                            this.form.messageError = response.data.msj
                            
                            return false
                        }
                        this.form.withMessageError = false
